 ...
 * Description:       Offer bitcoin as a payment option on your website and get
 * access to even more clients. Receive payments in bitcoins or in your local
 * currency, directly in your bank account. Enjoy an easy setup, no bitcoin
 * expertise required. Powered by TripleA.
 * Version: 1.3.1
 * Author: TripleA
...
function triplea_payment_gateway_for_woocommerce_run() {
   ...
   add_action('rest_api_init', function () {
      ...
      register_rest_route('triplea/v1', '/tx_update/(?P<token>[a-zA-Z0-9-_]+)', [
         'methods'  => 'POST',
...
function triplea_handle_api_tx_update(WP_REST_Request $request) {
   ...
   if ($api_endpoint_token === $token) {
      ...
      return new WP_Error('bad_token', 'Bad token, should be: ' . $api_endpoint_token, ['status'   => 403, 'settings' => $plugin_settings]);
   }
